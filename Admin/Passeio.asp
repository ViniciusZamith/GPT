...
'		on error resume next
	...
	SqlCreate = "INSERT INTO gptmysql.Passeio " &_
           "(NomePasseio" &_
           ",DescricaoPasseio" &_
           ",QtdeMinima" &_
           ",QtdeMaxima" &_
    ...
    <tbody>
		<%		  
  			...
  			if (Request("Action")="Busca") then
				...
				sqlSearch = "Select * from gptmysql.Passeio where NomePasseio like '%" & Request("Nome") &_ 
				...
				SqlLista = "Select * from gptmysql.Passeio"
			...
			do while not rsPasseio.eof
				Set rsTipo = Conn.Execute("Select * from gptmysql.TipoPasseio where IdTipo = " & rsPasseio("IdTipoPasseio"))
				DescTipo = rsTipo("DescricaoTipo")
				rsTipo.Close
				Set rsTipo = Nothing
																				  
				Set rsUF = Conn.Execute("Select * from gptmysql.UF where UF = '" & rsPasseio("Uf") & "';")
				UF = rsUF("UF")
				rsUF.Close
				Set rsUF = Nothing																				 
    	%>
